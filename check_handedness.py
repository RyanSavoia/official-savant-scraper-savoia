...
from pybaseballstats import statcast_pitch_arsenals_leaderboard, statcast_pitch_arsenal_stats_leaderboard
...
pitcher_data = statcast_pitch_arsenals_leaderboard(2024, min_ip=20)
print(f"   - Total columns: {len(pitcher_data.columns)}")

# Check for handedness-related columns
hand_cols = [col for col in pitcher_data.columns if any(x in col.lower() for x in ['rhb', 'lhb', 'rh_', 'lh_', 'hand'])]
...
batter_pitch_data = statcast_pitch_arsenal_stats_leaderboard(2024, min_pa=50)
...
hand_cols2 = [col for col in batter_pitch_data.columns if any(x in col.lower() for x in ['hand', 'bats', 'throws'])]
